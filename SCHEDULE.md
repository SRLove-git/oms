...
| 阶段 6 工程完善 | ✅ 已完成 | 前端 i18n/暗色主题、移动端 H5、OpenAPI（springdoc + 网关聚合）、Sentinel 流控、商城开放 API、核心链路并发加固、前后端测试补强（详见 README 阶段六） |
...
| 2026-08-14 | 阶段 6（工程完善）完成：前端 i18n/暗色主题（admin/portal/mobile）、移动端 H5、springdoc OpenAPI + 网关聚合 Swagger UI、Sentinel 流控（网关 + 订单/支付）、商城对接开放 API（HMAC 签名 + 幂等下单 + 商品/库存查询，`docs/open-api.md`）、核心链路并发加固（条件状态流转 + 回调金额校验 + 退款/支付幂等边界）、前后端测试补强（后端全量 74 用例通过，前端 Vitest 接入）；ADR-0003 记录决策 |
